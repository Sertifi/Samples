...
<html>
    ...
    List<ServiceDescription> importedServiceDescriptions = new List<ServiceDescription>();
    List<XmlSchema> importedSchemas = new List<XmlSchema>();
    ...
    string GetSoapOperationInput(bool soap12) {
		...
		string absPath = "[SOAP address not available]";
        ...
        if (port != null)
        {
            ...
            if (!string.IsNullOrEmpty(soapAddress.Location))
            {
    ...
    OperationBinding SoapOperationBinding {
        get { 
            if (soapOperationBinding == null)
                soapOperationBinding = FindBinding(typeof(System.Web.Services.Description.SoapBinding));
    ...
    void Page_Load(object sender, EventArgs e) {
        ...
        LoadExternals(serviceDescriptions, schemas);
        ...
        foreach (ServiceDescription importedSd in importedServiceDescriptions)
        {
            serviceDescriptions.Remove(importedSd);
        }

        foreach (XmlSchema schema in importedSchemas)
        {
            schemas.Remove(schema);
        }
    }
    
    void LoadExternals(ServiceDescriptionCollection sdc, XmlSchemas schemas)
    {                
        foreach (ServiceDescription sd in sdc)
        {
            foreach (Import import in sd.Imports)
            {
                string location;

                if (import.Location.IndexOfAny(new char[] { ':', '?', '*' }) < 0)
                {
                    // This is a relative path.
                    location = Server.MapPath(import.Location);
                }
                else
                {
                ...
                foreach (object osd in dcp.Documents.Values)
                {
                    ...
                    {                        
                        importedServiceDescriptions.Add((ServiceDescription)osd);   
                    }                        
                    else if (osd is XmlSchema)
                    {
                        schemas.Add((XmlSchema)osd);
                        importedSchemas.Add((XmlSchema)osd);
                    }
                }
            }  // foreach (Import import in sd)                       
        } // foreach (ServiceDescription sd in sdc)

        foreach (ServiceDescription isd in importedServiceDescriptions)
        {
            sdc.Add(isd);
        }
    }
    ...
    ServiceDescription GetWsdl(string location)
    {
        if (string.IsNullOrEmpty(location))
            return null;

        WebRequest request = (WebRequest)WebRequest.Create(location);
        request.Method = "GET";
        ...
        try
        {
            return ServiceDescription.Read(responseStream);
        }
        catch (Exception)
        {
            return null;
